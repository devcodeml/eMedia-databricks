...
def tmall_ztc_cumul_creative_etl(airflow_execution_date, run_id):
    ...
    first_row_data = tmall_ztc_cumul_creative_daily_df.first().asDict()
    ...
    tmall_ztc_cumul_creative_daily_df.union(tmall_ztc_cumul_creative_fail_df).createOrReplaceTempView(
        "tmall_ztc_cumul_creative_daily")
    ...
    tb_emedia_tmall_ztc_cumul_creative_mapping_fail = spark.table("mapping_fail_3") \
        .withColumn("etl_date", current_date()) \
        .withColumn("etl_create_time", current_timestamp()) \
        .dropDuplicates(tmall_ztc_cumul_creative_pks)
    tb_emedia_tmall_ztc_cumul_creative_mapping_fail.createOrReplaceTempView('all_mapping_fail')

    tb_emedia_tmall_ztc_cumul_creative_mapping_fail.write \
        .mode("overwrite") \
        .option("mergeSchema", "true") \
        .insertInto("stg.tb_emedia_tmall_ztc_cumul_creative_mapping_fail")

    # Query output result
    tb_emedia_tmall_ztc_cumul_creative_fail_df = spark.sql(f'''
            SELECT
                thedate as ad_date,
                category_id,
                brand_id,
    ...
    tb_emedia_tmall_ztc_cumul_creative_golden_df = tb_emedia_tmall_ztc_cumul_creative_df.union(
        tb_emedia_tmall_ztc_cumul_creative_fail_df)
    tb_emedia_tmall_ztc_cumul_creative_golden_df.createOrReplaceTempView('tb_emedia_tmall_ztc_cumul_creative')
    ...
    eab_db = spark.sql(f"""
        select  	direct_cart_total as directcarttotal,
                    ...
                    req_page_size as req_page_size,
                    req_effect as req_effect
                   from    tb_emedia_tmall_ztc_cumul_creative
               """)

    output_to_emedia(tb_emedia_tmall_ztc_cumul_creative_golden_df, f'{date}/{date_time}/ztc',
                     'EMEDIA_TMALL_ZTC_DAILY_CREATIVE_REPORT_NEW_FACT_CUMUL.CSV', dict_key='cumul')
