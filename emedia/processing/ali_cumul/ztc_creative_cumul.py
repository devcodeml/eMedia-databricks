...
def tmall_ztc_cumul_creative_etl_new(airflow_execution_date, run_id):
    '''
    ...
    stg_tmall_ztc_cumul_creative_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ztc_cumul_creativereport/tmall_ztc_cumul_creativereport_{file_date.strftime("%Y-%m-%d")}.csv.gz'

    log.info(f'stg_tmall_ztc_cumul_creative_path file: {stg_tmall_ztc_cumul_creative_path}')

    tmall_ztc_cumul_creative_daily_df = spark.read.csv(
        f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/{stg_tmall_ztc_cumul_creative_path}"
        , header=True
        , multiLine=True
        , sep="|"
    )

    tmall_ztc_cumul_creative_daily_df.withColumn("etl_date", F.current_date()).withColumn("etl_create_time", F.current_timestamp()).distinct().write.mode(
        "overwrite").insertInto("stg.ztc_creative_cumul_daily")
    ...
    #                         req_page_size as req_page_size,
    #                         req_effect as req_effect
    #                        from    ods.ztc_creative_cumul_daily
    #                    """)
    #
    # date = airflow_execution_date[0:10]
    ...
    tmall_ztc_creative_cumul_daily_df = spark.table("ods.ztc_creative_cumul_daily").drop('etl_create_time').drop('etl_update_time')
    tmall_ztc_creative_cumul_fail_df = spark.table("dwd.ztc_creative_cumul_daily_mapping_fail").drop('etl_date') \
        .drop('etl_create_time').drop('category_id').drop('brand_id')

    res = emedia_brand_mapping(spark, tmall_ztc_creative_cumul_daily_df.union(tmall_ztc_creative_cumul_fail_df), 'ztc')
    res[0].dropDuplicates(tmall_ztc_creative_pks).createOrReplaceTempView("all_mapping_success")


    # UPSERT DBR TABLE USING success mapping
    spark.sql("""
        MERGE INTO dwd.ztc_creative_cumul_daily_mapping_success

        USING all_mapping_success

        ON dwd.ztc_creative_cumul_daily_mapping_success.ad_date = all_mapping_success.ad_date

        AND dwd.ztc_creative_cumul_daily_mapping_success.adgroup_id = all_mapping_success.adgroup_id

        AND dwd.ztc_creative_cumul_daily_mapping_success.campaign_id = all_mapping_success.campaign_id

        AND dwd.ztc_creative_cumul_daily_mapping_success.creativeid = all_mapping_success.creativeid

        AND dwd.ztc_creative_cumul_daily_mapping_success.effect_days = all_mapping_success.effect_days

        AND dwd.ztc_creative_cumul_daily_mapping_success.store_id = all_mapping_success.store_id

        AND dwd.ztc_creative_cumul_daily_mapping_success.pv_type_in = all_mapping_success.pv_type_in
        
        AND dwd.ztc_creative_cumul_daily_mapping_success.campaign_type_name = all_mapping_success.campaign_type_name
    ...
    res[1].dropDuplicates(tmall_ztc_creative_pks) \
        .write \
        .mode("overwrite") \
        .insertInto("dwd.ztc_creative_cumul_daily_mapping_fail")


    spark.table("dwd.ztc_creative_cumul_daily_mapping_success").union(spark.table("dwd.ztc_creative_cumul_daily_mapping_fail"))\
        .drop('etl_date').drop('etl_create_time').drop('etl_source_table').createOrReplaceTempView("tmall_ztc_creative_cumul_daily")
    ...
    dwd_tmall_ztc_creative_daily_df = dwd_tmall_ztc_creative_daily_df.fillna('', subset=['mdm_productline_id'])
    dwd_tmall_ztc_creative_daily_df = dwd_tmall_ztc_creative_daily_df.withColumnRenamed("category_id", "emedia_category_id")
    dwd_tmall_ztc_creative_daily_df = dwd_tmall_ztc_creative_daily_df.withColumnRenamed("brand_id", "emedia_brand_id")
    dwd_tmall_ztc_creative_daily_df = dwd_tmall_ztc_creative_daily_df.withColumn('mdm_productline_id', update(dwd_tmall_ztc_creative_daily_df.mdm_productline_id))
    dwd_tmall_ztc_creative_daily_df.fillna('').distinct().write \
        .mode("overwrite") \
        .insertInto("dwd.ztc_creative_cumul_daily")



    spark.sql("delete from dwd.tb_media_emedia_ztc_cumul_daily_fact where report_level = 'creative' ")
    spark.table("dwd.ztc_creative_cumul_daily").selectExpr('ad_date','pv_type_in','ad_format_lv2','store_id','effect','effect_days'
