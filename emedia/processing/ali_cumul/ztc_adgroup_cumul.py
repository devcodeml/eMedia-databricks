...
def tmall_ztc_cumul_adgroup_etl_new(airflow_execution_date, run_id):
    '''
    ...
    tmall_ztc_cumul_adgroup_daily_df.withColumn("etl_date", F.current_date()).withColumn("etl_create_time",
                                                                                    F.current_timestamp()).distinct().write.mode(
        "overwrite").insertInto("stg.ztc_adgroup_cumul_daily")
    ...
    SELECT
                ...
                dw_create_time,
                dw_batch_number,
                'stg.ztc_adgroup_cumul_daily' as etl_source_table
        FROM stg.ztc_adgroup_cumul_daily
    """).withColumn("etl_create_time", F.current_timestamp()).withColumn("etl_update_time",
                                                                             F.current_timestamp()).distinct().write.mode(
        "overwrite"
    ).insertInto(
        "ods.ztc_adgroup_cumul_daily"
    )
    ...
    #                 req_page_size as req_page_size,
    #                 req_effect as req_effect
    #         from    ods.ztc_adgroup_cumul_daily
    #     """)
    #
    ...
    tmall_ztc_adgroup_cumul_daily_df = spark.table("ods.ztc_adgroup_cumul_daily").drop('etl_create_time').drop(
        'etl_update_time')
    tmall_ztc_adgroup_cumul_fail_df = spark.table("dwd.ztc_adgroup_cumul_daily_mapping_fail").drop('etl_date') \
        .drop('etl_create_time').drop('category_id').drop('brand_id')

    res = emedia_brand_mapping(spark, tmall_ztc_adgroup_cumul_daily_df.union(tmall_ztc_adgroup_cumul_fail_df), 'ztc')
    res[0].dropDuplicates(tmall_ztc_adgroup_pks).createOrReplaceTempView("all_mapping_success")
    spark.sql("""
               MERGE INTO dwd.ztc_adgroup_cumul_daily_mapping_success
               USING all_mapping_success
               ON dwd.ztc_adgroup_cumul_daily_mapping_success.ad_date = all_mapping_success.ad_date
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.campaign_id = all_mapping_success.campaign_id
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.adgroup_id = all_mapping_success.adgroup_id
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.effect_days = all_mapping_success.effect_days
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.store_id = all_mapping_success.store_id
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.pv_type_in = all_mapping_success.pv_type_in
               AND dwd.ztc_adgroup_cumul_daily_mapping_success.campaign_type_name = all_mapping_success.campaign_type_name
               WHEN MATCHED THEN
                   UPDATE SET *
               WHEN NOT MATCHED
                   THEN INSERT *
    ...
    res[1].dropDuplicates(tmall_ztc_adgroup_pks) \
        ...
        .insertInto("dwd.ztc_adgroup_cumul_daily_mapping_fail")

    spark.table("dwd.ztc_adgroup_cumul_daily_mapping_success").union(spark.table("dwd.ztc_adgroup_cumul_daily_mapping_fail")).drop(
    ...
    dwd_tmall_ztc_adgroup_cumul_daily_df = spark.sql("""
        ...
        ,case when campaign_name like '%定向%' then '定向词' when campaign_name like '%智能%' then '智能词' when campaign_name like '%销量明星%' then '销量明星' else '关键词' end as campaign_type
           ...
           ,'ods.ztc_adgroup_cumul_daily' as etl_source_table 
           from tmall_ztc_adgroup_cumul_daily a 
    ...
    dwd_tmall_ztc_adgroup_cumul_daily_df = dwd_tmall_ztc_adgroup_cumul_daily_df.fillna('', subset=['mdm_productline_id'])
    dwd_tmall_ztc_adgroup_cumul_daily_df = dwd_tmall_ztc_adgroup_cumul_daily_df.withColumnRenamed("category_id",
                                                                                      "emedia_category_id")
    dwd_tmall_ztc_adgroup_cumul_daily_df = dwd_tmall_ztc_adgroup_cumul_daily_df.withColumnRenamed("brand_id", "emedia_brand_id")
    dwd_tmall_ztc_daily_df = dwd_tmall_ztc_adgroup_cumul_daily_df.withColumn('mdm_productline_id', update(
        dwd_tmall_ztc_adgroup_cumul_daily_df.mdm_productline_id))
    dwd_tmall_ztc_daily_df.fillna('').distinct().write \
        .mode("overwrite") \
        .insertInto("dwd.ztc_adgroup_cumul_daily")

    spark.sql("delete from dwd.tb_media_emedia_ztc_cumul_daily_fact where report_level = 'adgroup' ")
    spark.table("dwd.ztc_adgroup_cumul_daily").selectExpr('ad_date', 'pv_type_in', 'ad_format_lv2', 'store_id', 'effect',
                                                    'effect_days'
