...
def tmall_ylmf_daliy_crowd_cumul_etl_new(airflow_execution_date):
    ...
    extend_json_content_df = tmall_ylmf_df.select("*", F.json_tuple("rpt_info", "add_new_charge", "add_new_uv",
    ...
    tmall_ylmf_crowd_daily_df = extend_json_content_df.na.fill("").selectExpr("log_data as ad_date", "campaign_group_id",
    ...
    (
        tmall_ylmf_crowd_daily_df.withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .distinct()
        .write.mode("overwrite")
        .insertInto("stg.ylmf_crowd_cumul_daily")
    )
    ...
    (
        spark.sql(
            ...
            dw_create_time,
            dw_batch_number
            from stg.ylmf_crowd_cumul_daily
            """
        )
        .withColumn("etl_source_table", F.lit("stg.ylmf_crowd_cumul_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .distinct()
        .write.mode("overwrite")
        .insertInto("ods.ylmf_crowd_cumul_daily")
    )
    ...
    tmall_ylmf_crowd_daily_df = (
        spark.table("ods.ylmf_crowd_cumul_daily").drop("etl_date").drop("etl_create_time").withColumnRenamed('store_id','req_storeId')
    )
    tmall_ylmf_crowd_daily_fail_df = (
        spark.table("dwd.ylmf_crowd_cumul_daily_mapping_fail")
        .drop("category_id")
        .drop("brand_id")
        .drop("etl_date")
    ...
    dwd_table = "dwd.ylmf_crowd_cumul_daily_mapping_success"
    ...
    (
        ...
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_crowd_cumul_daily_mapping_fail")
    )

    spark.table("dwd.ylmf_crowd_cumul_daily_mapping_success").union(
        spark.table("dwd.ylmf_crowd_cumul_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_crowd_cumul_daily")
    ...
    (
        ylmf_crowd_daily_res.distinct()
        .withColumn("etl_source_table", F.lit("ods.ylmf_crowd_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_crowd_cumul_daily")
    )
    ...
    #         t1.dw_create_time,
    #         t1.dw_batch_number,
    #         'dwd.ylmf_crowd_cumul_daily' as etl_source_table,
    #         current_timestamp() as etl_create_time,
    #         current_timestamp() as etl_update_time
    #     FROM
    #         dwd.ylmf_crowd_cumul_daily t1
    #     LEFT JOIN
