...
def tmall_ylmf_daliy_crowd_cumul_etl(airflow_execution_date, run_id):
    ...
    input_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ylmf_cumul_daily_displayreport/aliylmf_day_crowdReport_cumul_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    fail_table_exist = spark.sql(
        "show tables in stg like 'media_emedia_aliylmf_day_crowd_report_cumul_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_aliylmf_day_crowd_report_cumul_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    tmall_ylmf_mapping_pks = ['ad_date', 'campaign_group_id', 'campaign_id', 'promotion_entity_id', 'sub_crowd_value',
                              'effect_days', 'req_storeId', 'target_type']
    ...
    table_exist = spark.sql(
        "show tables in dws like 'media_emedia_aliylmf_day_crowd_report_cumul_mapping_success'").count()
    ...
    if table_exist == 0:
        all_success_df.write \
            ...
            .insertInto("dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success")
    else:
        spark.sql("""
          MERGE INTO dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success
          USING all_mapping_success
          ON dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.ad_date = all_mapping_success.ad_date
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.campaign_id = all_mapping_success.campaign_id
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.promotion_entity_id = all_mapping_success.promotion_entity_id
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.sub_crowd_value = all_mapping_success.sub_crowd_value
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.effect_days = all_mapping_success.effect_days
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.target_type = all_mapping_success.target_type
              AND dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.req_storeId = all_mapping_success.req_storeId
              AND ((dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                      OR
                   (dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
          WHEN MATCHED THEN
              UPDATE SET *
          WHEN NOT MATCHED
              THEN INSERT *
    ...
    all_fail_df.write \
        .mode("overwrite") \
        .option("mergeSchema", "true") \
        .insertInto("stg.media_emedia_aliylmf_day_crowd_report_cumul_mapping_fail")
    ...
    out_df = spark.sql(f'''
        ...
        from (
            select * from dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success where dw_batch_number = '{dw_batch_number}'
            union 
            select * from stg.media_emedia_aliylmf_day_crowd_report_mapping_fail where dw_batch_number = '{dw_batch_number}'
        )
    ''')

    # 输出GM
    output_to_emedia(out_df, f'{date}/{date_time}/ylmf_cuml', 'EMEDIA_TMALL_YLMF_CUML_DAILY_CROWD_REPORT_FACT.CSV',dict_key='cumul')

    # 输出eab 暂时不输出
    output_to_emedia(out_df,
                     f'fetchResultFiles/ALI_days/YLMF_CUMUL/{run_id}', f'tmall_ylmf_day_crowd_{date}.csv.gz',
                     dict_key='eab', compression='gzip', sep='|')

    spark.sql('optimize dws.media_emedia_aliylmf_day_crowd_report_cumul_mapping_success')
