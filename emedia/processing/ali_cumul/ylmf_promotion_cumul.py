...
def tmall_ylmf_daliy_promotion_cumul_etl_mew(airflow_execution_date):
    ...
    tmall_ylmf_promotion_daily_df = extend_json_content_df.na.fill("").selectExpr("log_data as ad_date", "campaign_group_id",
                                                            "campaign_group_name",
    ...
    (
        tmall_ylmf_promotion_daily_df.withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .distinct()
        .write.mode("overwrite")
        .insertInto("stg.ylmf_promotion_entity_cumul_daily")
    )
    ...
    (
        spark.sql(
            ...
            dw_create_time,
            dw_batch_number
            from stg.ylmf_promotion_entity_cumul_daily
            """
        )
        .withColumn("etl_source_table", F.lit("stg.ylmf_promotion_entity_cumul_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .distinct()
        .write.mode("overwrite")
        .insertInto("ods.ylmf_promotion_entity_cumul_daily")
    )
    ...
    tmall_ylmf_promotion_daily_df = (
        spark.table("ods.ylmf_promotion_entity_cumul_daily").drop("etl_date").drop("etl_create_time").withColumnRenamed('store_id','req_storeId')
    )

    # load unmapped records
    tmall_ylmf_promotion_daily_fail_df = (
        spark.table("dwd.ylmf_promotion_entity_cumul_daily_mapping_fail")
        .drop("category_id")
        .drop("brand_id")
        .drop("etl_date")
    ...
    dwd_table = "dwd.ylmf_promotion_entity_cumul_daily_mapping_success"
    ...
    (
        ...
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_promotion_entity_cumul_daily_mapping_fail")
    )

    # dwd.ylmf_promotion_daily
    spark.table("dwd.ylmf_promotion_entity_cumul_daily_mapping_success").union(
        spark.table("dwd.ylmf_promotion_entity_cumul_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_promotion_entity_cumul_daily")
    ...
    ylmf_promotion_daily_res = spark.sql(
        ...
        from ylmf_promotion_entity_cumul_daily a
    ...
    (
        ylmf_promotion_daily_res.distinct()
        .withColumn("etl_source_table", F.lit("ods.ylmf_promotion_entity_cumul_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_promotion_entity_cumul_daily")
    )
    ...
    #
    # tables = [
    #     "dwd.ylmf_promotion_entity_cumul_daily"
    # ]
    #
    # reduce(
    #     DataFrame.union,
    ...
    #         .withColumn("etl_source_table", F.lit(table)),
    #         tables,
    #     ),
    # ).createOrReplaceTempView("ylmf_promotion_entity_cumul_daily")
    #
    ...
    #     FROM
    #         ylmf_promotion_entity_cumul_daily t1
    #     LEFT JOIN
