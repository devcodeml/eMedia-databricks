...
def tmall_ylmf_campaign_group_cumul_etl_new(airflow_execution_date):
    ...
    (
        extend_json_content_df.na.fill("").selectExpr("log_data as ad_date",
                                                            "campaign_group_id",
                                                            "campaign_group_name",
        ...
        .insertInto("stg.ylmf_campaign_group_cumul_daily")
    )
    ...
    (
        spark.sql(
            ...
            from stg.ylmf_campaign_group_cumul_daily
            """
        )
        .withColumn("etl_source_table", F.lit("stg.ylmf_campaign_group_cumul_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .distinct()
        .write.mode("overwrite")
        .insertInto("ods.ylmf_campaign_group_cumul_daily")
    )
    ...
    tmall_ylmf_campaign_group_daily_df = (
        spark.table("ods.ylmf_campaign_group_cumul_daily")
        .drop("etl_date")
        .drop("etl_create_time").withColumnRenamed('store_id','req_storeId')
    )
    tmall_ylmf_campaign_group_daily_fail_df = (
        spark.table("dwd.ylmf_campaign_group_cumul_daily_mapping_fail")
        .drop("category_id")
        .drop("brand_id")
        .drop("etl_date")
    ...
    dwd_table = "dwd.ylmf_campaign_group_cumul_daily_mapping_success"
    ...
    (
        ...
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_campaign_group_cumul_daily_mapping_fail")
    )

    spark.table("dwd.ylmf_campaign_group_cumul_daily_mapping_success").union(
        spark.table("dwd.ylmf_campaign_group_cumul_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_campaign_group_cumul_daily")
    ...
    ylmf_campaign_group_daily_res = spark.sql(
        ...
        from ylmf_campaign_group_cumul_daily a
    ...
    (
        ylmf_campaign_group_daily_res.distinct()
        .withColumn("etl_source_table", F.lit("ods.ylmf_campaign_group_cumul_daily"))
        .withColumn("etl_date", F.current_date())
        .withColumn("etl_create_time", F.current_timestamp())
        .write.mode("overwrite")
        .insertInto("dwd.ylmf_campaign_group_cumul_daily")
    )
    ...
    spark.sql(
        ...
        SELECT
            ...
            dw_create_time,
            dw_batch_number,
            'dwd.ylmf_campaign_group_cumul_daily' as etl_source_table,
            current_timestamp() as etl_create_time,
            current_timestamp() as etl_update_time
        FROM
            dwd.ylmf_campaign_group_cumul_daily
        """
