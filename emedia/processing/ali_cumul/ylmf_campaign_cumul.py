...
def tmall_ylmf_campaign_cumul_etl_new(airflow_execution_date):
    ...
    tmall_ylmf_campaign_daily_df = extend_json_content_df.na.fill("").selectExpr("log_data as ad_date", "campaign_group_id",
    ...
    tmall_ylmf_campaign_daily_df.withColumn("etl_date", F.current_date())\
        .withColumn("etl_create_time", F.current_timestamp())\
        .distinct().write.mode("overwrite").insertInto("stg.ylmf_campaign_cumul_daily")
    ...
    tmall_ylmf_campaign_daily_df = (
        spark.table("ods.ylmf_campaign_cumul_daily").drop("etl_date").withColumnRenamed('store_id','req_storeId')
    ...
    (
        ylmf_campaign_daily_res.distinct()
