...
def tmall_ztc_cumul_keyword_etl(airflow_execution_date, run_id):
    ...
    tmall_ztc_cumul_keyword_daily_df.union(tmall_ztc_cumul_keyword_fail_df).createOrReplaceTempView(
        "tmall_ztc_cumul_keyword_daily")
    ...
    tb_emedia_tmall_ztc_cumul_keyword_fail_df = spark.table("mapping_fail_3") \
        .withColumn("etl_date", current_date()) \
        .withColumn("etl_create_time", current_timestamp()) \
        .dropDuplicates(tmall_ztc_cumul_keyword_pks)

    tb_emedia_tmall_ztc_cumul_keyword_fail_df.createOrReplaceTempView('all_mapping_fail')

    tb_emedia_tmall_ztc_cumul_keyword_fail_df.write \
        .mode("overwrite") \
        .option("mergeSchema", "true") \
        .insertInto("stg.tb_emedia_tmall_ztc_cumul_keyword_mapping_fail")

    # Query output result
    tb_emedia_tmall_ztc_cumul_keyword_fail_df = spark.sql(f'''
            SELECT
                thedate as ad_date,
                category_id,
                brand_id,
    ...
    tb_emedia_tmall_ztc_cumul_keyword_golden_df = tb_emedia_tmall_ztc_cumul_keyword_df.union(
        tb_emedia_tmall_ztc_cumul_keyword_fail_df)
    tb_emedia_tmall_ztc_cumul_keyword_golden_df.createOrReplaceTempView('tb_emedia_tmall_ztc_cumul_keyword')
    ...
    output_to_emedia(tb_emedia_tmall_ztc_cumul_keyword_golden_df, f'{date}/{date_time}/ztc',
                     'EMEDIA_TMALL_ZTC_DAILY_KEYWORD_REPORT_NEW_FACT_CUMUL.CSV', dict_key='cumul')
