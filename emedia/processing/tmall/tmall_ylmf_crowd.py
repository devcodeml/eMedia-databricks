...
def tmall_ylmf_daliy_crowd_etl(airflow_execution_date, run_id):
    ...
    fail_table_exist = spark.sql("show tables in stg like 'media_emedia_aliylmf_day_crowd_report_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_aliylmf_day_crowd_report_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    table_exist = spark.sql("show tables in dws like 'media_emedia_aliylmf_day_crowd_report_mapping_success'").count()
    # AND dws.media_emedia_tmall_ylmf_day_campaignGroup_mapping_success.effect_days = all_mappint_success.effect_days
    if table_exist == 0:
        res[0].write.mode("overwrite").option("mergeSchema", "true").insertInto("dws.media_emedia_aliylmf_day_crowd_report_mapping_success")
    else:
        spark.sql("""
          MERGE INTO dws.media_emedia_aliylmf_day_crowd_report_mapping_success
          USING all_mapping_success
          ON dws.media_emedia_aliylmf_day_crowd_report_mapping_success.ad_date = all_mapping_success.ad_date
              AND dws.media_emedia_aliylmf_day_crowd_report_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
              AND dws.media_emedia_aliylmf_day_crowd_report_mapping_success.campaign_id = all_mapping_success.campaign_id
              ...
              AND ((dws.media_emedia_aliylmf_day_crowd_report_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                      OR
                   (dws.media_emedia_aliylmf_day_crowd_report_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
          WHEN MATCHED THEN
              UPDATE SET *
          WHEN NOT MATCHED
              THEN INSERT *
        """)
    res[1].write.mode("overwrite").option("mergeSchema", "true").insertInto("stg.media_emedia_aliylmf_day_crowd_report_mapping_fail")
    # 全量输出
    update_time = F.udf(lambda x: x.replace("-", ""), StringType())
    success_output_df = spark.sql(
        "select * from dws.media_emedia_aliylmf_day_crowd_report_mapping_success where ad_date >= '{0}'".format(
            days_ago912)).drop('etl_date').drop('etl_create_time').withColumn('ad_date', update_time(F.col('ad_date')))
    fail_output_df = spark.sql(
        "select * from stg.media_emedia_aliylmf_day_crowd_report_mapping_fail where ad_date >= '{0}'".format(
            days_ago912)).drop('etl_date').drop('etl_create_time').withColumn('ad_date', update_time(F.col('ad_date')))
    ...
    eab_success_out_df = spark.sql(f'''
        ...
        from dws.media_emedia_aliylmf_day_crowd_report_mapping_success where dw_batch_number = '{dw_batch_number}'
    ''')
    ...
    eab_fail_out_df = spark.sql(f'''
            select 
                ...
                etl_date as dw_etl_date,
                '{run_id}' as dw_batch_id
            from stg.media_emedia_aliylmf_day_crowd_report_mapping_fail where dw_batch_number = '{dw_batch_number}'
        ''')
    ...
    spark.sql('optimize dws.media_emedia_aliylmf_day_crowd_report_mapping_success')
