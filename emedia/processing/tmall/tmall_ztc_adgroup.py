...
tmall_ztc_adgroup_mapping_success_tbl = 'dws.tb_emedia_tmall_ztc_adgroup_mapping_success'
tmall_ztc_adgroup_mapping_fail_tbl = 'stg.tb_emedia_tmall_ztc_adgroup_mapping_fail'
...
def tmall_ztc_adgroup_etl(airflow_execution_date,run_id):
    '''
    ...
    tmall_ztc_adgroup_daily_df = spark.read.csv(
    ...
    first_row_data = tmall_ztc_adgroup_daily_df.first().asDict()
    dw_batch_number = first_row_data.get('dw_batch_number')
    
    tmall_ztc_adgroup_fail_df = spark.table("stg.tb_emedia_tmall_ztc_adgroup_mapping_fail") \
                .drop('data_source') \
                .drop('dw_etl_date') \
                .drop('dw_batch_id') \
                .drop('category_id') \
    ...
    tmall_ztc_adgroup_daily_df.union(tmall_ztc_adgroup_fail_df).createOrReplaceTempView("tmall_ztc_adgroup_daily")
    ...
    mapping_1_result_df = spark.sql(f'''
        SELECT
            ...
            mapping_1.brand_id  
        FROM tmall_ztc_adgroup_daily t1 LEFT JOIN mapping_1 ON t1.req_storeId = mapping_1.account_id
    ''')
    ...
    tmall_ztc_adgroup_mapped_df = spark.table("mapping_success_1") \
                .union(spark.table("mapping_success_2")) \
                .union(spark.table("mapping_success_3")) \
                .withColumn("etl_date", current_date()) \
                .withColumn("etl_create_time", current_timestamp()) \
                .dropDuplicates(tmall_ztc_adgroup_pks)
                
    tmall_ztc_adgroup_mapped_df.createOrReplaceTempView("all_mapping_success")

    # UPSERT DBR TABLE USING success mapping
    spark.sql("""
        MERGE INTO dws.tb_emedia_tmall_ztc_adgroup_mapping_success

        USING all_mapping_success

        ON dws.tb_emedia_tmall_ztc_adgroup_mapping_success.req_start_time = all_mapping_success.req_start_time
        ...
        AND dws.tb_emedia_tmall_ztc_adgroup_mapping_success.req_pv_type_in = all_mapping_success.req_pv_type_in
    ...
    spark.table("mapping_fail_3") \
        ...
        .insertInto("stg.tb_emedia_tmall_ztc_adgroup_mapping_fail")


    # Query output result
    tb_emedia_tmall_ztc_adgroup_df = spark.sql(f'''
        SELECT
        ...
        FROM (
            SELECT *
            FROM dws.tb_emedia_tmall_ztc_adgroup_mapping_success 
                UNION
            SELECT *
            FROM stg.tb_emedia_tmall_ztc_adgroup_mapping_fail
        )
        WHERE req_start_time >= '{days_ago912}' AND req_start_time <= '{etl_date}'
    ''').dropDuplicates(output_tmall_ztc_adgroup_pks)

    tb_emedia_tmall_ztc_adgroup_df.createOrReplaceTempView("tb_emedia_tmall_ztc_adgroup")
    gm_db = tb_emedia_tmall_ztc_adgroup_df.drop('dw_batch_number')
    ...
    eab_db = spark.sql(f"""
            select  direct_cart_total as directcarttotal,
                    ...
                    req_page_size as req_page_size,
                    req_effect as req_effect
            from    tb_emedia_tmall_ztc_adgroup   where dw_batch_number = '{dw_batch_number}' and dw_batch_id = '{run_id}'
        """)

    output_to_emedia(gm_db, f'{date}/{date_time}/ztc', 'EMEDIA_TMALL_ZTC_DAILY_ADGROUP_REPORT_NEW_FACT.CSV')

    output_to_emedia(eab_db, f'fetchResultFiles/ALI_days/ZTC/{run_id}', f'tmall_ztc_day_adgroup_{date}.csv.gz',dict_key='eab', compression = 'gzip', sep='|')

    spark.sql("optimize dws.tb_emedia_tmall_ztc_adgroup_mapping_success")
