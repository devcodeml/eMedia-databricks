...
def tmall_ylmf_daliy_promotion_etl(airflow_execution_date, run_id):
    ...
    report_df = read_json_content_df.na.fill("").selectExpr("log_data as ad_date", "campaign_group_id",
                                                          ...
                                                          "`eq_report_query.group_by_log_date` as group_by_log_date",
    ...
    if table_exist == 0:
        res[0].distinct().write.mode("overwrite").saveAsTable(
            "dws.media_emedia_aliylmf_day_promotion_report_mapping_success")
    else:
        spark.sql("""
              ...
              ON dws.media_emedia_aliylmf_day_promotion_report_mapping_success.ad_date = all_mapping_success.ad_date
                  ...
                  AND dws.media_emedia_aliylmf_day_promotion_report_mapping_success.store_id = all_mapping_success.req_storeId
                  AND ((dws.media_emedia_aliylmf_day_promotion_report_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                          OR
    ...
    res[1].dropDuplicates(tmall_ylmf_campaign_pks).write.mode("overwrite").saveAsTable(
        "stg.media_emedia_aliylmf_day_promotion_report_mapping_fail")
    # 全量输出
