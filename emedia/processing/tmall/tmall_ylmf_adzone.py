...
def tmall_ylmf_daliy_adzone_etl(airflow_execution_date, run_id):
    ...
    input_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ylmf_daily_displayreport/aliylmf_day_adzoneReport_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    read_json_content_df = tmall_ylmf_df.select("*", F.json_tuple("rpt_info",
    ...
    fail_table_exist = spark.sql(
        "show tables in stg like 'media_emedia_aliylmf_day_adzone_report_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_aliylmf_day_adzone_report_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    table_exist = spark.sql(
        "show tables in dws like 'media_emedia_aliylmf_day_adzone_report_mapping_success'").count()
    # AND dws.media_emedia_tmall_ylmf_day_campaignGroup_mapping_success.effect_days = all_mappint_success.effect_days
    if table_exist == 0:
        res[0].write.mode("overwrite").option("mergeSchema", "true").insertInto("dws.media_emedia_aliylmf_day_adzone_report_mapping_success")
    else:
        spark.sql("""
              MERGE INTO dws.media_emedia_aliylmf_day_adzone_report_mapping_success
              USING all_mapping_success
              ON dws.media_emedia_aliylmf_day_adzone_report_mapping_success.ad_date = all_mapping_success.ad_date
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.campaign_id = all_mapping_success.campaign_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.adzone_id = all_mapping_success.adzone_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.promotion_entity_id = all_mapping_success.promotion_entity_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.effect_days = all_mapping_success.effect_days
                  AND dws.media_emedia_aliylmf_day_adzone_report_mapping_success.req_storeId = all_mapping_success.req_storeId
                  AND ((dws.media_emedia_aliylmf_day_adzone_report_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                          OR
                       (dws.media_emedia_aliylmf_day_adzone_report_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
              WHEN MATCHED THEN
                  UPDATE SET *
              WHEN NOT MATCHED
                  THEN INSERT *
            """)
    res[1].write.mode("overwrite").option("mergeSchema", "true").insertInto("stg.media_emedia_aliylmf_day_adzone_report_mapping_fail")
    # 全量输出
    update_time = F.udf(lambda x: x.replace("-", ""), StringType())
    success_output_df = spark.sql(
        "select * from dws.media_emedia_aliylmf_day_adzone_report_mapping_success where ad_date >= '{0}'".format(
            days_ago912)).drop('etl_date').drop('etl_create_time').withColumn('ad_date', update_time(F.col('ad_date')))
    fail_output_df = spark.sql(
        "select * from stg.media_emedia_aliylmf_day_adzone_report_mapping_fail where ad_date >= '{0}'".format(
            days_ago912)).drop('etl_date').drop('etl_create_time').withColumn('ad_date', update_time(F.col('ad_date')))
    ...
    output_to_emedia(all_output.withColumnRenamed('req_storeId', 'store_id'), f'{date}/{date_time}/ylmf',
                     'EMEDIA_TMALL_YLMF_DAILY_ADZONE_REPORT_FACT.CSV')

    # 增量输出
    ## 引用mapping函数 路径不一样自行修改函数路径
    eab_success_output_df = spark.sql(f'''
        select 
