...
def tmall_ylmf_daliy_adzone_cumul_etl(airflow_execution_date, run_id):
    ...
    fail_table_exist = spark.sql(
        "show tables in stg like 'media_emedia_aliylmf_day_adzone_report_cumul_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_aliylmf_day_adzone_report_cumul_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    table_exist = spark.sql(
        "show tables in dws like 'media_emedia_aliylmf_day_adzone_report_cumul_mapping_success'").count()
    if table_exist == 0:
        all_success_df.write. \
            mode("overwrite"). \
            option("mergeSchema", "true") \
            .insertInto("dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success")
    else:
        spark.sql("""
              MERGE INTO dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success
              USING all_mapping_success
              ON dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.ad_date = all_mapping_success.ad_date
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.campaign_id = all_mapping_success.campaign_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.adzone_id = all_mapping_success.adzone_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.promotion_entity_id = all_mapping_success.promotion_entity_id
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.effect_days = all_mapping_success.effect_days
                  AND dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.req_storeId = all_mapping_success.req_storeId
                  AND ((dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                          OR
                       (dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
              WHEN MATCHED THEN
                  UPDATE SET *
              WHEN NOT MATCHED
                  THEN INSERT *
    ...
    all_fail_df.write \
        .mode("overwrite") \
        .option("mergeSchema", "true") \
        .insertInto("stg.media_emedia_aliylmf_day_adzone_report_cumul_mapping_fail")
    ...
    out_df = spark.sql(f'''
        select 
         ...
         from (
             select * from dws.media_emedia_aliylmf_day_adzone_report_cumul_mapping_success where dw_batch_number = '{dw_batch_number}'
             union 
             select * from stg.media_emedia_aliylmf_day_adzone_report_cumul_mapping_fail where dw_batch_number = '{dw_batch_number}'
         )
    ''')
