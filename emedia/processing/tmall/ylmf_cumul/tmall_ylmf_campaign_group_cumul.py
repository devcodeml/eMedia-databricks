...
def tmall_ylmf_campaign_group_cumul_etl(airflow_execution_date, run_id):
    ...
    input_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ylmf_cumul_daily_displayreport/aliylmf_day_campaignGroup_cumul_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    read_json_content_df = tmall_ylmf_df.select("*",
                                                F.json_tuple("rpt_info", "add_new_charge", "add_new_uv",
                                                             ...
                                                             "prepay_inshop_amt",
                                                             "prepay_inshop_num", "return_pv", "return_pv_cost",
                                                             "roi",
                                                             "search_click_cnt", "search_click_cost").alias(
                                                    "add_new_charge",
                                                    "add_new_uv",
                                                    "add_new_uv_cost",
                                                    "add_new_uv_rate",
                                                    ...
                                                    "search_click_cnt", "search_click_cost")).drop('rpt_info')

    report_df = read_json_content_df.na.fill("").selectExpr("log_data as ad_date",
                                                            "campaign_group_id",
                                                            "campaign_group_name",
    ...
    fail_table_exist = spark.sql(
        "show tables in stg like 'media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    table_exist = spark.sql(
        "show tables in dws like 'media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success'").count()
    if table_exist == 0:
        res[0].writemode("overwrite").option("mergeSchema", "true").insertInto(
            "dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success")
    else:
        spark.sql("""
          MERGE INTO dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success
          USING all_mapping_success
          ON dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.ad_date = all_mapping_success.ad_date
              AND dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
              AND dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.effect_days = all_mapping_success.effect_days
              AND dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.req_storeId = all_mapping_success.req_storeId
              AND ((dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                      OR
                   (dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
          WHEN MATCHED THEN
              UPDATE SET *
          WHEN NOT MATCHED
              THEN INSERT *
        """)
    res[1].write.mode("overwrite").option("mergeSchema", "true").insertInto("stg.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_fail")
    ...
    out_df = spark.sql(f'''
        ...
        from ( 
            select * from dws.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_success where dw_batch_number = '{dw_batch_number}'
            union 
            select * from stg.media_emedia_tmall_ylmf_day_campaign_group_cumul_mapping_fail where dw_batch_number = '{dw_batch_number}'
        )
    ''')

    #输出GM
    output_to_emedia(out_df, f'{date}/{date_time}/ylmf_cuml','EMEDIA_TMALL_YLMF_CUML_DAILY_CAMPAIGN_GROUP_REPORT_FACT.CSV',dict_key='cumul')

    # 输出eab(暂时不需要)
    output_to_emedia(out_df,f'fetchResultFiles/ALI_days/YLMF_CUMUL/{run_id}', f'tmall_ylmf_day_campaign_group_{date}.csv.gz',
