...
def tmall_ylmf_daliy_creativepackage_cumul_etl(airflow_execution_date, run_id):
    ...
    input_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ylmf_cuml_daily_displayreport/aliylmf_day_creativePackageReport_cuml_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    output_to_emedia(out_df, f'{date}/{date_time}/ylmf_cuml',
                     'EMEDIA_TMALL_YLMF_CUML_DAILY_CREATIVEPACKAGE_REPORT_FACT.CSV')
    # eab
