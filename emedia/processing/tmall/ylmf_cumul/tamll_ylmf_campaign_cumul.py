...
def tmall_ylmf_campaign_cumul_etl(airflow_execution_date, run_id):
    ...
    ## 路径
    input_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/ylmf_cuml_daily_displayreport/tmall_ylmf_displayReport_cuml_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    first_row_data = tmall_ylmf_cumul_adgroupreport_df.first().asDict()
    dw_batch_number = first_row_data.get('dw_batch_number')

    base_data_df = tmall_ylmf_cumul_adgroupreport_df.select("*",
    ...
    report_df = base_data_df.na.fill("").selectExpr("log_data as ad_date", "campaign_group_id",
                                                    ...
                                                    "req_effect_type as effect_type",
    ...
    fail_table_exist = spark.sql(
        "show tables in stg like 'media_emedia_tmall_ylmf_campaign_report_cumul_mapping_fail'").count()
    if fail_table_exist == 0:
        daily_reports = report_df
    else:
        fail_df = spark.table("stg.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_fail") \
            .drop('category_id') \
            .drop('brand_id') \
            .drop('etl_date') \
    ...
    table_exist = spark.sql(
        "show tables in dws like 'media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success'").count()
    if table_exist == 0:
        all_success_df.write.mode("overwrite").option("mergeSchema", "true").insertInto(
            "dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success")
    else:
        spark.sql("""
          MERGE INTO dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success
          USING all_mapping_success
          ON dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.ad_date = all_mapping_success.ad_date
              AND dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id
              AND dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.campaign_id = all_mapping_success.campaign_id
              AND dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.effect_days = all_mapping_success.effect_days
              AND dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.req_storeId = all_mapping_success.req_storeId
              AND ((dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.campaign_group_id = all_mapping_success.campaign_group_id)
                      OR
                   (dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success.campaign_group_id IS null and all_mapping_success.campaign_group_id IS null))
          WHEN MATCHED THEN
              UPDATE SET *
          WHEN NOT MATCHED
              THEN INSERT *
        """)
    all_fail_df = res[1]
    all_fail_df.write.mode("overwrite").option("mergeSchema", "true").insertInto(
        "stg.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_fail")
    ...
    out_df = spark.sql(f'''
        select
            ...
            from (
                select * from dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success where dw_batch_number = '{dw_batch_number}'
                union 
                select * from stg.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_fail where dw_batch_number = '{dw_batch_number}'
            )
   ''')
    # 输出GM
    output_to_emedia(out_df, f'{date}/{date_time}/ylmf_cuml', 'EMEDIA_TMALL_YLMF_CUML_DAILY_CAMPAIGN_REPORT_FACT.CSV')

    # 输出eab(暂时不需要)
    # output_to_emedia(out_df, f'fetchResultFiles/ALI_days/YLMF_CUMUL/{run_id}',
    #                  f'tmall_ylmf_day_campaign_{date}.csv.gz', dict_key='eab', compression='gzip',sep='|')
    spark.sql('optimize dws.media_emedia_tmall_ylmf_campaign_report_cumul_mapping_success')
    return 0
