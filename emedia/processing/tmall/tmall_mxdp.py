...
tmall_mxdp_campaign_mapping_success_tbl = 'dws.tb_emedia_tmall_mxdp_campaign_mapping_success'
tmall_mxdp_campaign_mapping_fail_tbl = 'stg.tb_emedia_tmall_mxdp_campaign_mapping_fail'


tmall_mxdp_campaign_pks = [
    'thedate'
    , 'req_storeId'
    , 'campaignid'
    , 'adgroupid'
...
output_tmall_mxdp_campaign_pks = [
    'ad_date'
    , 'store_id'
    , 'campaign_id'
    , 'adgroup_id'
...
# to specify date range
    ...
    tmall_mxdp_campaign_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall/mxdp_daily_adgroupreport/tmall_mxdp_adgroupreport_{file_date.strftime("%Y-%m-%d")}.csv.gz'

    log.info(f'tmall_mxdp campaign file: {tmall_mxdp_campaign_path}')

    tmall_mxdp_campaign_daily_df = spark.read.csv(
                    f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/{tmall_mxdp_campaign_path}"
                    , header = True
                    , multiLine = True
                    , sep = "|"
    )
    
    tmall_mxdp_campaign_fail_df = spark.table("stg.tb_emedia_tmall_mxdp_campaign_mapping_fail") \
                .drop('category_id') \
                .drop('brand_id') \
                .drop('etl_date') \
    ...
    tmall_mxdp_campaign_daily_df.union(tmall_mxdp_campaign_fail_df).createOrReplaceTempView("tmall_mxdp_campaign_daily")
    ...
    mapping_1_result_df = spark.sql('''
        SELECT
            ...
            mapping_1.brand_id  
        FROM tmall_mxdp_campaign_daily LEFT JOIN mapping_1 ON tmall_mxdp_campaign_daily.req_storeId = mapping_1.account_id
    ''')
    ...
    tmall_mxdp_mapped_df = spark.table("mapping_success_1") \
                .union(spark.table("mapping_success_2")) \
                .union(spark.table("mapping_success_3")) \
                .withColumn("etl_date", current_date()) \
                .withColumn("etl_create_time", current_timestamp()) \
                .dropDuplicates(tmall_mxdp_campaign_pks)
                
    tmall_mxdp_mapped_df.createOrReplaceTempView("all_mapping_success")

    # UPSERT DBR TABLE USING success mapping
    spark.sql("""
        MERGE INTO dws.tb_emedia_tmall_mxdp_campaign_mapping_success

        USING all_mapping_success

        ON dws.tb_emedia_tmall_mxdp_campaign_mapping_success.thedate = all_mapping_success.thedate

        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.req_storeId = all_mapping_success.req_storeId
        
        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.campaignid = all_mapping_success.campaignid
        
        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.adgroupid = all_mapping_success.adgroupid
        
        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.req_effect = all_mapping_success.req_effect
        
        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.req_effect_Days = all_mapping_success.req_effect_Days
        
        AND dws.tb_emedia_tmall_mxdp_campaign_mapping_success.req_traffic_type = all_mapping_success.req_traffic_type
    ...
    spark.table("mapping_fail_3") \
        ...
        .insertInto("stg.tb_emedia_tmall_mxdp_campaign_mapping_fail")


    # Query output result
    tb_emedia_tmall_mxdp_campaign_df = spark.sql(f'''
        SELECT
            ...
            click_transactionshipping as order_quantity ,
            click_transactiontotal as order_value ,
        ...
        FROM (
            SELECT *
            FROM dws.tb_emedia_tmall_mxdp_campaign_mapping_success WHERE thedate >= '{days_ago912}' AND thedate <= '{etl_date}'
                UNION
            SELECT *
            FROM stg.tb_emedia_tmall_mxdp_campaign_mapping_fail
        )
        WHERE thedate >= '{days_ago912}'
              AND thedate <= '{etl_date}'
    ''').dropDuplicates(output_tmall_mxdp_campaign_pks)

    output_to_emedia(tb_emedia_tmall_mxdp_campaign_df, f'{date}/{date_time}/mxdp', 'TB_EMEDIA_TMALL_MXDP_ADGROUP_FACT.CSV')
