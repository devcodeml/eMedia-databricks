...
def jd_zw_creative_etl(airflow_execution_date,run_id):
    ...
    '''
    spark = get_spark()
    airflow_execution_date = '2022-09-21'
...
# qa
    ...
    jd_dmp_creative_daily_df.withColumn("data_source", F.lit('jingdong.ads.ibg.UniversalJosService.ad.query')).withColumn("dw_batch_id", F.lit(run_id)).withColumn("dw_etl_date", F.current_date()).distinct().write.mode(
        "overwrite").insertInto("stg.jdzw_creative_daily")



    # ods.jdzw_creative_daily
    ...
    spark.sql("""
        ...
        "overwrite").insertInto("ods.jdzw_creative_daily")
    ...
    jdzw_creative_df = spark.table('ods.jdzw_creative_daily').drop('dw_etl_date')
    jdzw_creative_fail_df = spark.table("dwd.jdzw_creative_daily_mapping_fail").drop('category_id').drop(
        'brand_id').drop('etl_date').drop('etl_create_time')

    res = emedia_brand_mapping(spark, jdzw_creative_df.union(jdzw_creative_fail_df), 'dmp')
    res[0].dropDuplicates(jdzw_creative_pks).createOrReplaceTempView("all_mapping_success")

    spark.sql("""
                MERGE INTO dwd.jdzw_creative_daily_mapping_success
                USING all_mapping_success
                ON dwd.jdzw_creative_daily_mapping_success.ad_date = all_mapping_success.ad_date
                AND dwd.jdzw_creative_daily_mapping_success.campaign_id = all_mapping_success.campaign_id
                AND dwd.jdzw_creative_daily_mapping_success.pin_name = all_mapping_success.pin_name
                AND dwd.jdzw_creative_daily_mapping_success.effect_days = all_mapping_success.effect_days
                AND dwd.jdzw_creative_daily_mapping_success.ad_group_id = all_mapping_success.ad_group_id
                AND dwd.jdzw_creative_daily_mapping_success.ad_id = all_mapping_success.ad_id
                AND dwd.jdzw_creative_daily_mapping_success.mobile_type = all_mapping_success.mobile_type
                AND dwd.jdzw_creative_daily_mapping_success.business_type = all_mapping_success.business_type
                AND dwd.jdzw_creative_daily_mapping_success.gift_flag = all_mapping_success.gift_flag
                AND dwd.jdzw_creative_daily_mapping_success.order_status_category = all_mapping_success.order_status_category
                AND dwd.jdzw_creative_daily_mapping_success.click_or_order_caliber = all_mapping_success.click_or_order_caliber
                AND dwd.jdzw_creative_daily_mapping_success.impression_or_click_effect = all_mapping_success.impression_or_click_effect
                WHEN MATCHED THEN
                    UPDATE SET *
                WHEN NOT MATCHED
                    THEN INSERT *
    ...
    res[1].dropDuplicates(jdzw_creative_pks) \
        .write \
        .mode("overwrite") \
    ...
    spark.table("dwd.jdzw_creative_daily_mapping_success").union(
        spark.table("dwd.jdzw_creative_daily_mapping_fail")).createOrReplaceTempView("jdzw_creative_daily")
    ...
    jdzw_campaign_daily_res.selectExpr('ad_date', "'品牌聚效' as ad_format_lv2", 'pin_name', 'effect', 'effect_days',
                                       'campaign_id', 'campaign_name',"campaign_subtype", "ad_group_id as adgroup_id", "ad_group_name as adgroup_name",
                                       "'creative' as report_level", "'' as report_level_id", "'' as report_level_name",
                                       'emedia_category_id', 'emedia_brand_id', 'mdm_category_id', 'mdm_brand_id',
                                       'mdm_productline_id', "'' as delivery_version",
                                        'mobile_type', 'business_type',
                                       'gift_flag', 'order_status_category', 'click_or_order_caliber',
                                       "'' as put_type", "'' as campaign_put_type", 'campaign_type', "cost", 'clicks as click',
                                       'impressions', 'order_quantity', 'order_value',
                                       'total_cart_quantity', 'new_customer_quantity', "data_source as dw_source",
                                       'dw_create_time',
        ...
        .write.mode("overwrite").insertInto("dwd.jdzw_creative_daily")
