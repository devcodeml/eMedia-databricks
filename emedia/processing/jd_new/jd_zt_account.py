...
def jd_jdzt_account_etl(airflow_execution_date,run_id):
    '''
...
# qa
    ...
    jd_jdzt_account_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/jd/dmp_daily_Report/jd_dmp_campaign_{file_date.strftime("%Y-%m-%d")}.csv.gz'

    log.info("jd_jdzt_account_path: " + jd_jdzt_account_path)

    jd_jdzt_account_daily_df = spark.read.csv(
        f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/{jd_jdzt_account_path}"
        , header=True
        , multiLine=True
        , sep="|"
    )

    jd_jdzt_account_daily_df.withColumn("data_source", F.lit('jingdong.ads.ibg.UniversalJosService.campaign.query')).withColumn("dw_batch_id", F.lit(run_id)).withColumn("dw_etl_date", F.current_date()).distinct().write.mode(
        "overwrite").insertInto("stg.jdzt_account_daily")
    ...
    spark.sql("""
        ...
        "overwrite").insertInto("ods.jdzt_account_daily")
