...
def tmall_ztc_target_etl(airflow_execution_date, run_id):
    ...
    stg_tmall_ztc_target_daily_df = spark.read.csv(
    ...
    stg_tmall_ztc_target_daily_df.withColumn("etl_date", F.current_date()).withColumn("etl_create_time", F.current_timestamp()).distinct().write.mode(
        "overwrite").insertInto("stg.ztc_target_daily")
    ...
    tmall_ztc_target_daily_df = spark.table("ods.ztc_target_daily").drop('etl_create_time').drop('etl_update_time')
    tmall_ztc_target_fail_df = spark.table("dwd.ztc_target_daily_mapping_fail").drop('etl_date') \
        .drop('etl_create_time').drop('category_id').drop('brand_id')

    res = emedia_brand_mapping(spark, tmall_ztc_target_daily_df.union(tmall_ztc_target_fail_df), 'ztc')
    res[0].dropDuplicates(tmall_ztc_target_pks).createOrReplaceTempView("all_mapping_success")


    spark.sql("""
                MERGE INTO dwd.ztc_target_daily_mapping_success
                USING all_mapping_success
                ON dwd.ztc_target_daily_mapping_success.ad_date = all_mapping_success.ad_date
                AND dwd.ztc_target_daily_mapping_success.campaign_id = all_mapping_success.campaign_id
                AND dwd.ztc_target_daily_mapping_success.adgroup_id = all_mapping_success.adgroup_id
                AND dwd.ztc_target_daily_mapping_success.effect_days = all_mapping_success.effect_days
                AND dwd.ztc_target_daily_mapping_success.store_id = all_mapping_success.store_id
                AND dwd.ztc_target_daily_mapping_success.crowd_id = all_mapping_success.crowd_id
                AND dwd.ztc_target_daily_mapping_success.pv_type_in = all_mapping_success.pv_type_in
                WHEN MATCHED THEN
                    UPDATE SET *    
                WHEN NOT MATCHED
                    THEN INSERT *
    ...
    res[1].dropDuplicates(tmall_ztc_target_pks) \
        .write \
        .mode("overwrite") \
        .insertInto("dwd.ztc_target_daily_mapping_fail")
    ...
    spark.table("dwd.ztc_target_daily_mapping_success").union(spark.table("dwd.ztc_target_daily_mapping_fail"))\
        .drop('etl_date').drop('etl_create_time').drop('etl_source_table').drop('campaign_type').createOrReplaceTempView("tmall_ztc_target_daily")
    ...
    dwd_tmall_ztc_target_daily_df = spark.sql("""
        ...
        ,b.localProductLineId as mdm_productline_id,d.category2_code as emedia_category_id,c.brand_code as emedia_brand_id
        ,'ods.ztc_target_daily' as etl_source_table 
        from tmall_ztc_target_daily a 
    ...
    dwd_tmall_ztc_target_daily_df.fillna('').distinct().write \
        .mode("overwrite") \
        .insertInto("dwd.ztc_target_daily")



    spark.sql("delete from dwd.tb_media_emedia_ztc_daily_fact where report_level = 'crowd' ")
    spark.table("dwd.ztc_target_daily").selectExpr('ad_date','pv_type_in','ad_format_lv2','store_id','effect','effect_days'
                                        ...
                                        ,"'' as niname",'emedia_category_id','emedia_brand_id','category_id','brand_id'
