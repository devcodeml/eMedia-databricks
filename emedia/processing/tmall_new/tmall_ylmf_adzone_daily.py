...
def tmall_ylmf_adzone_daily_etl(airflow_execution_date, run_id):
    ...
    # daily report
    tmall_ylmf_adzone_daily_path = (
        f"fetchResultFiles/{file_date.strftime('%Y-%m-%d')}/tmall"
        "/ylmf_daily_displayreport/aliylmf_day_adzoneReport_"
        f"{file_date.strftime('%Y-%m-%d')}.csv.gz"
    )

    origin_ylmf_adzone_daily_df = spark.read.csv(
        f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/"
        f"{tmall_ylmf_adzone_daily_path}",
        header=True,
        multiLine=True,
        sep="|",
        quote='"',
    ...
    # extend json content
    extend_json_content_df = origin_ylmf_adzone_daily_df.select(
        "*",
        json_tuple(
            "rpt_info",
            "add_new_charge",
    ...
    tmall_ylmf_adzone_daily_df = extend_json_content_df.selectExpr(
        ...
        "cast(adzone_id as string) as adzone_id",
        "cast(adzone_name as string) as adzone_name",
        ...
        "cast(`req_report_query.campaign_id_list` as string) as campaign_id_list",
    ...
    # stg.ylmf_adzone_daily
    (
        tmall_ylmf_adzone_daily_df.withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("stg.ylmf_adzone_daily")
        .insertInto("stg.ylmf_adzone_daily")
    )

    # ods.ylmf_adzone_daily
    # 1. 数据清洗（去重，空值处理）
    # 2. 数据标准化（日期，枚举格式的标准化）
    (
        spark.sql(
            ...
            select
                ...
                dw_create_time,
                dw_batch_number
            from stg.ylmf_adzone_daily
            """
        )
        .withColumn("etl_source_table", lit("stg.ylmf_adzone_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("ods.ylmf_adzone_daily")
        .insertInto("ods.ylmf_adzone_daily")
    )
    ...
    #    "overwrite"
    # ).saveAsTable(
    #    "dwd.ylmf_adzone_daily_mapping_success"
    # )
    ...
    dwd_table = "dwd.ylmf_adzone_daily_mapping_success"
    ...
    (
        ...
        .insertInto("dwd.ylmf_adzone_daily_mapping_fail")
        # .saveAsTable("dwd.ylmf_adzone_daily_mapping_fail")
    )

    spark.table("dwd.ylmf_adzone_daily_mapping_success").union(
        spark.table("dwd.ylmf_adzone_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_adzone_daily")
    ...
    (
        ylmf_adzone_daily_res.selectExpr(
            "ad_date",
        ...
        .distinct()
        .withColumn("etl_source_table", lit("ods.ylmf_adzone_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        # .saveAsTable("dwd.ylmf_adzone_daily")
        .insertInto("dwd.ylmf_adzone_daily")
    )
    ...
    tables = [
        "dwd.ylmf_adzone_daily",
        "dwd.feedflow_adzone_daily",
        "dwd.super_zz_adzone_daily",
    ]
...
def tmall_feedflow_adzone_fact_old_stg_etl():
    emedia_conf_dict = get_emedia_conf_dict()
    ...
    emedia_tmall_feedflow_adzone_source_df = (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_feedflow_adzone_daily")
        .option("user", username)
        .option("password", password)
        .load()
    )
    emedia_tmall_feedflow_adzone_source_df.distinct().write.mode("overwrite").option(
        "mergeSchema", "true"
    ).saveAsTable("stg.feedflow_adzone_daily")


def tmall_feedflow_adzone_fact_old_dwd_etl():
    tmall_ylmf_adzone_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
    ...
    (
        spark.sql(
            ...
            select
                ...
                cast(a.adzone_id as string) as adzone_id,
                cast(a.adzone_name as string) as adzone_name,
                ...
                'stg.feedflow_adzone_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.feedflow_adzone_daily a
            left join ods.media_category_brand_mapping c
                on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            """
        )
        .dropDuplicates(tmall_ylmf_adzone_daily_pks)
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.feedflow_adzone_daily")
    )


def tmall_zz_super_adzone_fact_old_stg_etl():
    emedia_conf_dict = get_emedia_conf_dict()
    server_name = emedia_conf_dict.get("server_name")
    database_name = emedia_conf_dict.get("database_name")
    username = emedia_conf_dict.get("username")
    ...
    # password = "1qaZcde3"
    url = server_name + ";" + "databaseName=" + database_name + ";"
    tb_emedia_tmall_zz_super_adzone_source_df = (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_zz_super_adzone_fact")
        .option("user", username)
        .option("password", password)
        .load()
    )
    tb_emedia_tmall_zz_super_adzone_source_df.distinct().write.mode("overwrite").option(
        "mergeSchema", "true"
    ).saveAsTable("stg.super_zz_adzone_daily")


def tmall_zz_super_adzone_fact_old_dwd_etl():
    tmall_ylmf_adzone_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
    ...
    (
        spark.sql(
            ...
            select
                ...
                cast(c.category2_code as string) as mdm_category_id,
                cast(c.brand_code as string) as mdm_brand_id,
                'stg.super_zz_adzone_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.super_zz_adzone_daily a
            left join ods.media_category_brand_mapping c
                on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            """
        ...
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.super_zz_adzone_daily")
    )
