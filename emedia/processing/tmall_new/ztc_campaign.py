...
def tmall_ztc_campaign_etl_new(airflow_execution_date, run_id):
    ...
    tmall_ztc_campaign_daily_df.withColumn("etl_date", F.current_date()).withColumn("etl_create_time",
                                                                                    F.current_timestamp()).distinct().write.mode(
        "overwrite").insertInto("stg.ztc_campaign_daily")
    ...
    tmall_ztc_campaign_daily_df = spark.table("ods.ztc_campaign_daily").drop('etl_create_time').drop('etl_update_time')
    tmall_ztc_campaign_fail_df = spark.table("dwd.ztc_campaign_daily_mapping_fail").drop('etl_date') \
        .drop('etl_create_time').drop('category_id').drop('brand_id')

    res = emedia_brand_mapping(spark, tmall_ztc_campaign_daily_df.union(tmall_ztc_campaign_fail_df), 'ztc')
    ...
    spark.sql("""
        MERGE INTO dwd.ztc_campaign_daily_mapping_success

        USING all_mapping_success

        ON dwd.ztc_campaign_daily_mapping_success.ad_date = all_mapping_success.ad_date

        AND dwd.ztc_campaign_daily_mapping_success.campaign_id = all_mapping_success.campaign_id

        AND dwd.ztc_campaign_daily_mapping_success.effect_days = all_mapping_success.effect_days

        AND dwd.ztc_campaign_daily_mapping_success.store_id = all_mapping_success.store_id

        AND dwd.ztc_campaign_daily_mapping_success.pv_type_in = all_mapping_success.pv_type_in
    ...
    dwd_tmall_ztc_campaign_daily_df = dwd_tmall_ztc_campaign_daily_df.withColumnRenamed("category_id", "emedia_category_id")
    dwd_tmall_ztc_campaign_daily_df = dwd_tmall_ztc_campaign_daily_df.withColumnRenamed("brand_id", "emedia_brand_id")
