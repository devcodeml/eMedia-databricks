...
def tmall_ylmf_crowd_daily_etl(airflow_execution_date, run_id):
    ...
    # daily report
    tmall_ylmf_crowd_daily_path = (
        f"fetchResultFiles/{file_date.strftime('%Y-%m-%d')}/tmall"
        "/ylmf_daily_displayreport/aliylmf_day_crowdReport_"
        f"{file_date.strftime('%Y-%m-%d')}.csv.gz"
    )

    origin_ylmf_crowd_daily_df = spark.read.csv(
        f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/"
        f"{tmall_ylmf_crowd_daily_path}",
        header=True,
        multiLine=True,
        sep="|",
        quote='"',
    ...
    # extend json content
    extend_json_content_df = origin_ylmf_crowd_daily_df.select(
        "*",
        json_tuple(
            "rpt_info",
            "add_new_charge",
    ...
    tmall_ylmf_crowd_daily_df = extend_json_content_df.selectExpr(
        ...
        "cast(sub_crowd_value as string) as sub_crowd_value",
    ...
    # stg.ylmf_crowd_daily
    (
        tmall_ylmf_crowd_daily_df.withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("stg.ylmf_crowd_daily")
        .insertInto("stg.ylmf_crowd_daily")
    )
    ...
    (
        spark.sql(
            ...
            select
                ...
                dw_create_time,
                dw_batch_number
            from stg.ylmf_crowd_daily
            """
        )
        .withColumn("etl_source_table", lit("stg.ylmf_crowd_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("ods.ylmf_crowd_daily")
        .insertInto("ods.ylmf_crowd_daily")
    )

    tmall_ylmf_crowd_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
        "promotion_entity_id",
    ...
    (
        ylmf_crowd_daily_mapping_success,
        ylmf_crowd_daily_mapping_fail,
    ) = emedia_brand_mapping(
        # spark, tmall_ylmf_crowd_daily_df, "ylmf"
        spark,
        tmall_ylmf_crowd_daily_df.union(tmall_ylmf_crowd_daily_fail_df),
        "ylmf",
    )

    # ylmf_crowd_daily_mapping_success.dropDuplicates(
    #  tmall_ylmf_crowd_daily_pks
    # ).write.mode(
    #    "overwrite"
    # ).saveAsTable(
    #    "dwd.ylmf_crowd_daily_mapping_success"
    # )
    ...
    dwd_table = "dwd.ylmf_crowd_daily_mapping_success"
    tmp_table = "all_mapping_success"
    and_str = " AND ".join(
        [
            f"{dwd_table}.{col} <=> {tmp_table}.{col}"
            for col in tmall_ylmf_crowd_daily_pks
        ]
    )
    spark.sql(
        f"""
    ...
    (
        ylmf_crowd_daily_mapping_fail.dropDuplicates(tmall_ylmf_crowd_daily_pks)
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        # .saveAsTable("dwd.ylmf_crowd_daily_mapping_fail")
        .insertInto("dwd.ylmf_crowd_daily_mapping_fail")
    )

    spark.table("dwd.ylmf_crowd_daily_mapping_success").union(
        spark.table("dwd.ylmf_crowd_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_crowd_daily")

    ylmf_crowd_daily_res = spark.sql(
        """
        select
            a.*,
    ...
    (
        ylmf_crowd_daily_res.selectExpr(
            "ad_date",
        ...
        .distinct()
        .withColumn("etl_source_table", lit("ods.ylmf_crowd_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        # .saveAsTable("dwd.ylmf_crowd_daily")
        .insertInto("dwd.ylmf_crowd_daily")
    )
    ...
    spark.sql(
        ...
        SELECT
            ...
            t1.target_type as report_level_id,
            t1.target_name as report_level_name,
            t1.sub_crowd_value as sub_crowd_name,
            '' as audience_name,
...
def tmall_feedflow_crowd_daily_stg_etl():
    log.info("tmall_feedflow_crowd_daily_stg_etl is processing")
    emedia_conf_dict = get_emedia_conf_dict()
    ...
    emedia_tmall_feedflow_crowd_source_df = (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_feedflow_crowd_fact")
        .option("user", username)
        .option("password", password)
        .load()
    )
    emedia_tmall_feedflow_crowd_source_df.distinct().write.mode("overwrite").option(
        "mergeSchema", "true"
    ).saveAsTable("stg.feedflow_crowd_daily")


def tmall_feedflow_crowd_daily_dwd_etl():
    log.info("tmall_feedflow_crowd_daily_dwd_etl is processing")
    tmall_ylmf_crowd_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
    ...
    (
        spark.sql(
            ...
            select
                ...
                '' as sub_crowd_value,
                cast(a.crowd_name as string) as target_name,
                cast(a.crowd_id as string) as target_type,
                ...
                'stg.feedflow_crowd_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.feedflow_crowd_daily a
            left join ods.media_category_brand_mapping c
                on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            """
        )
        .dropDuplicates(tmall_ylmf_crowd_daily_pks)
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.feedflow_crowd_daily")
    )


def tmall_super_zz_crowd_daily_stg_etl():
    log.info("tmall_super_zz_crowd_daily_stg_etl is processing")
    emedia_conf_dict = get_emedia_conf_dict()
    server_name = emedia_conf_dict.get("server_name")
    database_name = emedia_conf_dict.get("database_name")
    username = emedia_conf_dict.get("username")
    ...
    # password = "1qaZcde3"
    url = server_name + ";" + "databaseName=" + database_name + ";"
    tb_emedia_tmall_zz_super_crowd_source_df = (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_zz_super_crowd_fact")
        .option("user", username)
        .option("password", password)
        .load()
    )
    tb_emedia_tmall_zz_super_crowd_source_df.distinct().write.mode("overwrite").option(
        "mergeSchema", "true"
    ).saveAsTable("stg.super_zz_crowd_daily")


def tmall_super_zz_crowd_daily_dwd_etl():
    log.info("tmall_super_zz_crowd_daily_dwd_etl is processing")
    tmall_ylmf_crowd_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
    ...
    (
        spark.sql(
            ...
            select
                ...
                cast(c.category2_code as string) as mdm_category_id,
                cast(c.brand_code as string) as mdm_brand_id,
                'stg.super_zz_crowd_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.super_zz_crowd_daily a
            left join ods.media_category_brand_mapping c
              on a.brand_id = c.emedia_brand_code and
              a.category_id = c.emedia_category_code
            """
        ...
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.super_zz_crowd_daily")
    )


def tmall_zz_target_daily_stg_etl():
    log.info("tmall_zz_target_daily_stg_etl is processing")
    emedia_conf_dict = get_emedia_conf_dict()
    server_name = emedia_conf_dict.get("server_name")
    database_name = emedia_conf_dict.get("database_name")
    username = emedia_conf_dict.get("username")
    ...
    (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_zz_target_fact")
        .option("user", username)
        .option("password", password)
        .load()
        .distinct()
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("stg.zz_target_daily")
    )


def tmall_zz_target_daily_dwd_etl():
    log.info("tmall_zz_target_daily_dwd_etl is processing")
    tmall_ylmf_crowd_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
    ...
    (
        spark.sql(
            ...
            select
                ...
                '' as promotion_entity_name,
                '' as sub_crowd_value,
                cast(a.target_audience_name as string) as target_name,
                cast(a.target_audience_id as string) as target_type,
                '' as add_new_charge,
                ...
                cast(c.category2_code as string) as mdm_category_id,
                cast(c.brand_code as string) as mdm_brand_id,
                'stg.zz_target_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.zz_target_daily a
            left join ods.media_category_brand_mapping c
                on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            """
        ...
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.zz_target_daily")
    )
