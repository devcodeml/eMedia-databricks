...
def tmall_ylmf_promotion_daily_etl_new(airflow_execution_date):
    ...
    # daily report
    tmall_ylmf_promotion_daily_path = (
        f"fetchResultFiles/{file_date.strftime('%Y-%m-%d')}/tmall"
        f"/ylmf_daily_displayreport/aliylmf_day_promotionReport_"
        f"{file_date.strftime('%Y-%m-%d')}.csv.gz"
    )

    origin_ylmf_promotion_daily_df = spark.read.csv(
        f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/"
        f"{tmall_ylmf_promotion_daily_path}",
        header=True,
        multiLine=True,
        sep="|",
        quote='"',
    ...
    # extend json content
    extend_json_content_df = origin_ylmf_promotion_daily_df.select(
        "*",
        json_tuple(
            "rpt_info",
            "add_new_charge",
    ...
    tmall_ylmf_promotion_daily_df = extend_json_content_df.selectExpr(
    ...
    # stg.ylmf_promotion_daily
    (
        tmall_ylmf_promotion_daily_df.withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("stg.ylmf_promotion_daily")
        .insertInto("stg.ylmf_promotion_daily")
    )

    # ods.ylmf_promotion_daily
    # 1. 数据清洗（去重，空值处理）
    # 2. 数据标准化（日期，枚举格式的标准化）
    (
        spark.sql(
            ...
            select
                ...
                cast(promotion_entity_id as string) as promotion_entity_id,
                cast(promotion_entity_name as string) as promotion_entity_name,
                cast(add_new_charge as DECIMAL(20,8)) as add_new_charge,
                cast(add_new_uv as BIGINT) as add_new_uv,
                add_new_uv_cost,
                cast(add_new_uv_rate as DECIMAL(20,10)) as add_new_uv_rate,
                cast(alipay_inshop_amt as decimal(20, 10)) as alipay_inshop_amt,
                cast(alipay_inshop_num as bigint) as alipay_inshop_num,
                cast(avg_access_page_num as BIGINT) as avg_access_page_num,
                cast(avg_deep_access_times as DECIMAL(20,10)) as avg_deep_access_times,
                cast(cart_num as bigint) as cart_num,
                cast(charge as decimal(20, 4)) as charge,
                cast(click as bigint) as click,
                cast(cpc as DECIMAL(20,10)) as cpc,
                cast(cpm as DECIMAL(20,10)) as cpm,
                ctr,
                cast(cvr as DECIMAL(20,10)) as cvr,
                cast(deep_inshop_pv as DECIMAL(20,4)) as deep_inshop_pv,
                cast(dir_shop_col_num as bigint) as dir_shop_col_num,
                cast(gmv_inshop_amt as DECIMAL(20,8)) as gmv_inshop_amt,
                cast(gmv_inshop_num as bigint) as gmv_inshop_num,
                cast(icvr as DECIMAL(20,10)) as icvr,
                cast(impression as bigint) as impression,
                cast(inshop_item_col_num as bigint) as inshop_item_col_num,
                inshop_potential_uv,
                inshop_potential_uv_rate,
                cast(inshop_pv as bigint) as inshop_pv,
                inshop_pv_rate,
                cast(inshop_uv as DECIMAL(20,4)) as inshop_uv,
                prepay_inshop_amt,
                prepay_inshop_num,
                return_pv,
                return_pv_cost,
                cast(roi as DECIMAL(20,10)) as roi,
                search_click_cnt,
                search_click_cost,
                biz_code,
                offset,
                ...
                dw_create_time,
                dw_batch_number
            from stg.ylmf_promotion_daily
            """
        )
        .withColumn("etl_source_table", lit("stg.ylmf_promotion_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("ods.ylmf_promotion_daily")
        .insertInto("ods.ylmf_promotion_daily")
    )

    # mapping to dwd

    # union mapped data and remove duplicates
    tmall_ylmf_promotion_daily_pks = [
        "ad_date",
        "campaign_group_id",
        "campaign_id",
        "promotion_entity_id",
    ...
    tmall_ylmf_promotion_daily_fail_df = (
        spark.table("dwd.ylmf_promotion_daily_mapping_fail")
        .drop("category_id")
        .drop("brand_id")
        .drop("etl_date")
    ...
    (
        tmall_ylmf_promotion_mapped_success_df,
        tmall_ylmf_promotion_mapped_fail_df,
    ) = emedia_brand_mapping(
        #    spark, tmall_ylmf_promotion_daily_df, "ylmf"
        spark,
        tmall_ylmf_promotion_daily_df.union(tmall_ylmf_promotion_daily_fail_df),
        "ylmf",
    )

    # tmall_ylmf_promotion_mapped_success_df.dropDuplicates(
    #  tmall_ylmf_promotion_daily_pks
    # ).write.mode(
    #    "overwrite"
    # ).saveAsTable(
    #    "dwd.ylmf_promotion_daily_mapping_success"
    # )
    tmall_ylmf_promotion_mapped_success_df.dropDuplicates(
        tmall_ylmf_promotion_daily_pks
    ...
    dwd_table = "dwd.ylmf_promotion_daily_mapping_success"
    tmp_table = "all_mapping_success"
    and_str = " AND ".join(
        [
            f"{dwd_table}.{col} <=> {tmp_table}.{col}"
            for col in tmall_ylmf_promotion_daily_pks
        ]
    )
    spark.sql(
        f"""
    ...
    (
        tmall_ylmf_promotion_mapped_fail_df.dropDuplicates(
            tmall_ylmf_promotion_daily_pks
        )
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .insertInto("dwd.ylmf_promotion_daily_mapping_fail")
        # .saveAsTable("dwd.ylmf_promotion_daily_mapping_fail")
    )

    # dwd.ylmf_promotion_daily
    spark.table("dwd.ylmf_promotion_daily_mapping_success").union(
        spark.table("dwd.ylmf_promotion_daily_mapping_fail")
    ).createOrReplaceTempView("ylmf_promotion_daily")

    ylmf_promotion_daily_res = spark.sql(
        """
        select
            a.*,
    ...
    (
        ylmf_promotion_daily_res.selectExpr(
            "ad_date",
        ...
        .distinct()
        .withColumn("etl_source_table", lit("ods.ylmf_promotion_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        # .saveAsTable("dwd.ylmf_promotion_daily")
        .insertInto("dwd.ylmf_promotion_daily")
    )
...
def tmall_super_zz_adgroup_daily_stg_etl():
    ...
    (
        ...
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_zz_super_adgroup_fact")
        ...
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("stg.super_zz_adgroup_daily")
    )
...
def tmall_feedflow_adgroup_daily_stg_etl():
    ...
    (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option("url", url)
        .option("query", "select * from dbo.tb_emedia_tmall_adgroup_daily_fact")
        .option("user", username)
        .option("password", password)
        .load()
        .distinct()
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("stg.feedflow_adgroup_daily")
    )
    ...
    (
        spark.sql(
            ...
            select
                ...
                cast(null as DECIMAL(20,8)) as add_new_charge,
                cast(null as bigint) as add_new_uv,
                '' as add_new_uv_cost,
                cast(null as DECIMAL(20,10)) as add_new_uv_rate,
                cast(a.alipay_inshop_amt as decimal(20, 10)) as alipay_inshop_amt,
                cast(a.alipay_in_shop_num as bigint) as alipay_inshop_num,
                cast(a.avg_access_page_num as bigint) as avg_access_page_num,
                cast(a.avg_access_time as DECIMAL(20,10)) as avg_deep_access_times,
                cast(a.cart_num as bigint) as cart_num,
                cast(a.cost as decimal(20, 4)) as charge,
                cast(a.click as bigint) as click,
                cast(a.ecpc as DECIMAL(20,10)) as cpc,
                cast(a.ecpm as DECIMAL(20,10)) as cpm,
                cast(a.ctr as string) as ctr,
                cast(a.cvr as DECIMAL(20,10)) as cvr,
                cast(null as DECIMAL(20,4)) as deep_inshop_pv,
                cast(a.dir_shop_col_num as bigint) as dir_shop_col_num,
                cast(a.gmv_inshop_amt as DECIMAL(20,8)) as gmv_inshop_amt,
                cast(a.gmv_inshop_num as bigint) as gmv_inshop_num,
                cast(null as DECIMAL(20,10)) as icvr,
                cast(a.ad_pv as bigint) as impression,
                cast(a.inshop_item_col_num as bigint) as inshop_item_col_num,
                '' as inshop_potential_uv,
                '' as inshop_potential_uv_rate,
                cast(null as bigint) as inshop_pv,
                '' as inshop_pv_rate,
                cast(a.inshop_uv as DECIMAL(20,4)) as inshop_uv,
                '' as prepay_inshop_amt,
                '' as prepay_inshop_num,
                '' as return_pv,
                '' as return_pv_cost,
	            cast(a.roi as DECIMAL(20,10)) as roi,
                '' as search_click_cnt,
                ...
                'stg.super_zz_adgroup_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.super_zz_adgroup_daily a
            left join ods.media_category_brand_mapping c
            on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            where a.ad_date >= '2021-01-01'
            """
        )
        .dropDuplicates(tmall_ylmf_promotion_daily_stg_pks)
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .saveAsTable("dwd.super_zz_adgroup_daily")
    )
    ...
    (
        spark.sql(
            ...
            select
                ...
                '' as effect_type,
                case
                    when ad_date < '2020-10-15' and effect = 3 then 1
                ...
                '' as promotion_entity_id,
                '' as promotion_entity_name,
                cast(a.new_f_charge as DECIMAL(20,8)) as add_new_charge,
                cast(a.add_new_uv as bigint) as add_new_uv,
                '' as add_new_uv_cost,
                cast(a.add_new_uv_rate as DECIMAL(20,10)) as add_new_uv_rate,
                cast(a.alipay_inshop_amt as DECIMAL(20,10)) as alipay_inshop_amt,
                cast(a.alipay_in_shop_num as bigint) as alipay_inshop_num,
                cast(a.avg_access_page_num as bigint) as avg_access_page_num,
                cast(a.avg_access_time as DECIMAL(20,10)) as avg_deep_access_times,
                ...
                '' as ctr,
                cast(a.cvr as DECIMAL(20,10)) as cvr,
                cast(a.deep_inshop_num as DECIMAL(20,4)) as deep_inshop_pv,
                cast(a.follow_number as bigint) as dir_shop_col_num,
                cast(a.gmv_inshop_amt as DECIMAL(20,8)) as gmv_inshop_amt,
                cast(a.gmv_inshop_num as bigint) as gmv_inshop_num,
                cast(a.icvr as DECIMAL(20,10)) as icvr,
                cast(a.ad_pv as bigint) as impression,
                cast(a.inshop_item_col_num as bigint) as inshop_item_col_num,
                '' as inshop_potential_uv,
                '' as inshop_potential_uv_rate,
                cast(a.inshop_pv as bigint) as inshop_pv,
                '' as inshop_pv_rate,
                cast(a.inshop_uv as DECIMAL(20,4)) as inshop_uv,
                '' as prepay_inshop_amt,
                '' as prepay_inshop_num,
                ...
                cast(c.category2_code as string) as mdm_category_id,
                cast(c.brand_code as string) as mdm_brand_id,
                'stg.feedflow_adgroup_daily' as etl_source_table,
                current_date() as etl_date,
                current_timestamp() as etl_create_time
            from stg.feedflow_adgroup_daily a
            left join ods.media_category_brand_mapping c
                on a.brand_id = c.emedia_brand_code and
                a.category_id = c.emedia_category_code
            where (campaign_name like 'HC_HE%' and store_id='2017026')
                or store_id!='2017026'
            """
        )
        .dropDuplicates(tmall_ylmf_promotion_daily_stg_pks)
    ...
    (
        spark.sql(
            ...
            select
                ...
                '' as promotion_entity_name,
                cast(null as DECIMAL(20,8)) as add_new_charge,
                cast(a.uv as bigint) as add_new_uv,
                '' as add_new_uv_cost,
                cast(null as DECIMAL(20,10)) as add_new_uv_rate,
                cast(a.order_value as decimal(20, 10)) as alipay_inshop_amt,
                cast(a.order_quantity as bigint) as alipay_inshop_num,
                cast(a.avg_access_page_quantity as bigint) as avg_access_page_num,
                cast(a.avg_access_time_length as DECIMAL(20,10)) as avg_deep_access_times,
                cast(a.total_cart_quantity as bigint) as cart_num,
                cast(a.cost as decimal(20, 4)) as charge,
                cast(a.clicks as bigint) as click,
                cast(null as DECIMAL(20,10)) as cpc,
                cast(null as DECIMAL(20,10)) as cpm,
                '' as ctr,
                cast(null as DECIMAL(20,10)) as cvr,
                cast(a.deep_inshop_uv as DECIMAL(20,4)) as deep_inshop_pv,
                cast(null as bigint) as dir_shop_col_num,
                cast(a.gmv_value as DECIMAL(20,8)) as gmv_inshop_amt,
                cast(a.gmv_quantity as bigint) as gmv_inshop_num,
                cast(null as DECIMAL(20,10)) as icvr,
                cast(a.impressions as bigint) as impression,
                cast(null as bigint) as inshop_item_col_num,
                '' as inshop_potential_uv,
                '' as inshop_potential_uv_rate,
                cast(null as bigint) as inshop_pv,
                '' as inshop_pv_rate,
                cast(null as DECIMAL(20,4)) as inshop_uv,
                '' as prepay_inshop_amt,
                '' as prepay_inshop_num,
                '' as return_pv,
                '' as return_pv_cost,
	            cast(null as DECIMAL(20,10)) as roi,
                '' as search_click_cnt,
                '' as search_click_cost,
                '' as biz_code,
                '' as offset,
