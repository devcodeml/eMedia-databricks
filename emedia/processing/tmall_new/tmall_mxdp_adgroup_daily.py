...
def tmall_mxdp_adgroup_daily_etl(airflow_execution_date):
    # date
    ...
    # daily report
    tmall_mxdp_adgroup_path = (
        f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/tmall'
        "/mxdp_daily_adgroupreport/tmall_mxdp_adgroupreport_"
    ...
    (
        ...
        # .saveAsTable("stg.mxdp_adgroup_daily")
        .insertInto("stg.mxdp_adgroup_daily")
    )

    # ods.mxdp_adgroup_daily
    (
        spark.sql(
            """
            select
                ...
                cast(req_traffic_type as string) as req_traffic_type,
                req_start_date,
                req_end_date,
                case
            ...
            from stg.mxdp_adgroup_daily
            """
        )
        .distinct()
        .write.mode("overwrite")
        # .saveAsTable("ods.mxdp_adgroup_daily")
        .insertInto("ods.mxdp_adgroup_daily")
    )

    mxdp_adgroup_daily_df = (
        spark.table("ods.mxdp_adgroup_daily").drop("etl_date").drop("etl_create_time")
    )
    mxdp_adgroup_daily_fail_df = (
        spark.table("dwd.mxdp_adgroup_daily_mapping_fail")
        .drop("category_id")
        .drop("brand_id")
        .drop("etl_date")
    ...
    mxdp_adgroup_daily_df.union(mxdp_adgroup_daily_fail_df).createOrReplaceTempView(
        "mxdp_adgroup_daily"
    )
    # mxdp_adgroup_daily_df.createOrReplaceTempView("mxdp_adgroup_daily")
    ...
    # .saveAsTable("dwd.mxdp_adgroup_daily_mapping_success")

    # UPSERT DBR TABLE USING success mapping
    dwd_table = "dwd.mxdp_adgroup_daily_mapping_success"
    tmp_table = "all_mapping_success"
    and_str = " AND ".join(
        [f"{dwd_table}.{col} <=> {tmp_table}.{col}" for col in mxdp_adgroup_pks]
    )
    spark.sql(
        f"""
    ...
    (
        ...
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .dropDuplicates(mxdp_adgroup_pks)
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        .insertInto("dwd.mxdp_adgroup_daily_mapping_fail")
        # .saveAsTable("dwd.mxdp_adgroup_daily_mapping_fail")
    )

    spark.table("dwd.mxdp_adgroup_daily_mapping_success").union(
        spark.table("dwd.mxdp_adgroup_daily_mapping_fail")
    ).createOrReplaceTempView("mxdp_adgroup_daily")

    mxdp_adgroup_daily_res = spark.sql(
        """
        select
          a.*,
    ...
    (
        mxdp_adgroup_daily_res.selectExpr(
            "ad_date",
            "'明星店铺' as ad_format_lv2",
            "store_id",
            "effect",
            "effect_days",
            "req_traffic_type",
            ...
            "total_cart_quantity",
            "transaction_total",
            "cast(null as int) as order_quantity",
            "cast(null as decimal(19, 4)) as order_value",
            "click_roi",
            "click_uv",
            "uv",
            "uv_new",
        ...
        .distinct()
        .withColumn("etl_source_table", lit("ods.mxdp_adgroup_daily"))
        .withColumn("etl_date", current_date())
        .withColumn("etl_create_time", current_timestamp())
        .write.mode("overwrite")
        .option("mergeSchema", "true")
        # .saveAsTable("dwd.mxdp_adgroup_daily")
        .insertInto("dwd.mxdp_adgroup_daily")
    )

    mxdp_adgroup_daily_fact_pks = [
        "ad_date",
        "store_id",
        "effect",
        "effect_days",
    ...
    # dwd.tb_media_emedia_mxdp_daily_fact
    spark.sql(
        """
        delete from dwd.tb_media_emedia_mxdp_daily_fact
        where `report_level` = 'adgroup' 
        """
    )
    tables = ["dwd.mxdp_adgroup_daily"]
    ...
    spark.sql(
        ...
        FROM 
            mxdp_adgroup_daily
        """
    ).dropDuplicates(mxdp_adgroup_daily_fact_pks).write.mode("append").insertInto(
        "dwd.tb_media_emedia_mxdp_daily_fact"
    )
