...
def push_to_dw():
    ...
    spark.table("dwd.jdkc_adgroup_daily").distinct().write.mode("overwrite").format(
        "com.databricks.spark.sqldw"
    ).option("url", url).option("user", user).option("password", password).option(
        "forwardSparkAzureStorageCredentials", "true"
    ).option(
        "dbTable", "dbo.tb_emedia_jd_kc_adgroup_daily_v202209_fact"
    ).option(
        "tempDir", temp_dir
    ).save()
