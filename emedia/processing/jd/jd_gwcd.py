...
jd_gwcd_campaign_mapping_success_tbl = 'dws.tb_emedia_jd_gwcd_campaign_mapping_success'
jd_gwcd_campaign_mapping_fail_tbl = 'stg.tb_emedia_jd_gwcd_campaign_mapping_fail'
...
def jd_gwcd_campaign_etl(airflow_execution_date:str = ''):
    '''
    ...
    jd_gwcd_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/jd/gwcd_daily_campaignreport/jd_gwcd_campaignReport_{file_date.strftime("%Y-%m-%d")}.csv.gz'

    log.info("jd gwcd api file: " + jd_gwcd_path)

    jd_gwcd_daily_df = spark.read.csv(
                    f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/{jd_gwcd_path}"
                    , header = True
                    , multiLine = True
                    , sep = "|"
    )
    
    jd_gwcd_fail_df = spark.table("stg.tb_emedia_jd_gwcd_campaign_mapping_fail") \
                .drop('category_id') \
                .drop('brand_id') \
                .drop('etl_date') \
    ...
    jd_gwcd_daily_df.union(jd_gwcd_fail_df).createOrReplaceTempView("jd_gwcd_daily")
    ...
    jd_gwcd_mapped_df = spark.table("mapping_success_1") \
                .union(spark.table("mapping_success_2")) \
                .union(spark.table("mapping_success_3")) \
                .withColumn("etl_date", current_date()) \
                .withColumn("etl_create_time", current_timestamp()) \
                .dropDuplicates(jd_gwcd_campaign_pks)
                
    jd_gwcd_mapped_df.createOrReplaceTempView("all_mapping_success")

    # UPSERT DBR TABLE USING success mapping
    spark.sql("""
        MERGE INTO dws.tb_emedia_jd_gwcd_campaign_mapping_success

        USING all_mapping_success

        ON dws.tb_emedia_jd_gwcd_campaign_mapping_success.date = all_mapping_success.date

        AND dws.tb_emedia_jd_gwcd_campaign_mapping_success.pin = all_mapping_success.pin

        AND dws.tb_emedia_jd_gwcd_campaign_mapping_success.campaignId = all_mapping_success.campaignId

        AND dws.tb_emedia_jd_gwcd_campaign_mapping_success.req_clickOrOrderDay = all_mapping_success.req_clickOrOrderDay
    ...
    spark.table("mapping_fail_3") \
        ...
        .insertInto("stg.tb_emedia_jd_gwcd_campaign_mapping_fail")


    # Query output result
    tb_emedia_jd_gwcd_campaign_df = spark.sql(f'''
        SELECT
            date AS ad_date
            , pin AS pin_name
            , category_id
            ...
            , campaignTypeExpand AS campaign_type_expand
            , commentCnt
            , couponCnt
            , CPA
            , CPC
        ...
        FROM(
            SELECT *
            FROM dws.tb_emedia_jd_gwcd_campaign_mapping_success
                UNION
            SELECT *
            FROM stg.tb_emedia_jd_gwcd_campaign_mapping_fail
        )
        WHERE date >= '{days_ago912}'
              AND date <= '{curr_date}'
    ''').dropDuplicates(output_jd_gwcd_campaign_pks)

    output_to_emedia(tb_emedia_jd_gwcd_campaign_df, f'{output_date}/{output_date_time}/gwcd', 'EMEDIA_JD_GWCD_DAILY_CAMPAIGN_REPORT_FACT.CSV')
