...
def jd_ha_campaign_etl(airflow_execution_date:str = ''):
    ...
    jd_ha_campagin_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/jd/ha_daily_effectreport/jd-ha_reportGetEffectReportcsv_{file_date.strftime("%Y-%m-%d")}.csv.gz'

    log.info(f'jd ha campaign file: {jd_ha_campagin_path}')

    jd_ha_campagin_daily_df = spark.read.csv(
                    f"wasbs://{input_container}@{input_account}.blob.core.chinacloudapi.cn/{jd_ha_campagin_path}"
                    , header = True
                    , multiLine = True
                    , sep = "|"
    )
    ...
    jd_ha_campagin_daily_df.union(jd_ha_campaign_fail_df).createOrReplaceTempView("jd_ha_campaign_daily")
