...
def jd_sem_adgroup_etl(airflow_execution_date:str = ''):
    ...
    jd_jst_campagin_path = f'fetchResultFiles/{file_date.strftime("%Y-%m-%d")}/jd/jst_daily_campaignreport/jd_sem_adgroupReport_{file_date.strftime("%Y-%m-%d")}.csv.gz'
    ...
    jd_jst_campagin_daily_df.union(jd_sem_adgroup_fail_df).createOrReplaceTempView("jd_sem_adgroup_daily")
    ...
    mapping_1_result_df = spark.sql('''
        SELECT
            ...
            , mapping_1.brand_id
        FROM jd_sem_adgroup_daily LEFT JOIN mapping_1 ON jd_sem_adgroup_daily.req_pin = mapping_1.account_id
    ''')
    ...
    spark.sql("""
        ...
        AND dws.tb_emedia_jd_sem_adgroup_mapping_success.req_pin = all_mapping_success.req_pin
    ...
    output_to_emedia(tb_emedia_jd_sem_adgroup_df, f'{output_date}/{output_date_time}/jst', 'EMEDIA_JD_JST_DAILY_CAMPAIGN_REPORT_FACT.CSV')
