...
def jd_zt_adgroup_campaign_etl(airflow_execution_date:str = ''):
    ...
    tb_emedia_jd_zt_adgroup_campaign_df = spark.sql(f'''
        SELECT
            ...
            '{output_date_time}' as dw_batch_number,
            '{curr_date}' as dw_create_time,
            'jd' as dw_resource
        FROM (
            SELECT *
            FROM dws.tb_emedia_jd_zt_adgroup_campaign_mapping_success WHERE date >= '{days_ago912}' AND date <= '{curr_date}'
                UNION
            SELECT *
    ...
    output_to_emedia(tb_emedia_jd_zt_adgroup_campaign_df, f'{output_date}/{output_date_time}/jdzt', 'EMEDIA_JD_ZT_DAILY_ADGROUP_REPORT_FACT.CSV')
